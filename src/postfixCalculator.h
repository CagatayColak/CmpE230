...
void push(TokenStack *stack, Token item) {  // push an item to the stack
    stack->top++;
    stack->items[stack->top].name = item.name;
    stack->items[stack->top].type = item.type;
}

Token pop(TokenStack *stack, Token *postfix) {  // pop the top of the stack
...
Token peek(TokenStack *stack) { // peek at the top of the stack
...
Token *infixToPostfix(Token *infix, int infixSize, int *error) {
    ...
    for (i = 0, j = 0; i < infixSize; i++) {    // iterate through the infix expression
...
int isEmpty(TokenStack *stack) {    // check if the stack is empty
    return stack->top == -1;
}
...
void pushPostfix(TokenStack *stack, LLI item) { // push an item to the stack
    stack->top++;
...
LLI leftRotate(LLI n, LLI d) {  // Function to left rotate n by d bits
...
LLI rightRotate(LLI n, LLI d) { // Function to right rotate n by d bits
